...
const vylety = [
	{
		...
		stroller: 'partly',
		distance: "2,2",
		popis: 'You will see a gate and behind it a childrens playground at "Robertova vila", we recommend starting here from the train station. From the playground, then return to the station and cross the road at the end of the tracks. You will reach a sidewalk along the river. Follow it to the left of the station towards the high chimney. He cannot be overlooked. From the chimney, you will see the local castle. It is not accessible, but you will find a large field with mouflons. Access to the park is the gateway to the Penny Store. The garden is beautiful and worth a visit. From the park then continue to the many playgrounds on the street "Družba", they are hidden between the houses but explore them. From the playgrounds, we recommend going back to the main square. You will find the court just behind the big bridge. At its upper end, you will see the town hall. Right next to it is an excellent bakery where you can buy coffee and tortillas. At the same time, a green tourist sign starts at the bakery. If you follow it towards the church and further up the hills, you will reach a lookout tower called the Acacia Tower. On the tourist signpost, the lookout tower is marked under the name "Výhon". The journey to the Acacia Tower / "Výhon" lookout tower takes about 40 minutes. Distance from the square is 2 km,it leads through a beautiful landscape with unique forests and statues in the countryside.',
...

	{
		...
		stroller: 'no',
		distance: '2,2',
		popis: 'For lovers of secrets of forest, games and beautiful views: The Educational forest path in Kuřim.Follow the description of the journey in the post.    ',
		foto: 'kurim.jpg',
	},
...
appElm.innerHTML = SeznamVyletu( { seznam: vylety,	nadpis: 'Best family hikes ever' } );
...
const hledej = (event) => {
	...
	const vysledekHledani = vylety.filter(
		(vyletik) => vyletik.nazev.toLowerCase().includes(dotaz.toLowerCase()) 
	);

	appElm.innerHTML = SeznamVyletu( { seznam: vysledekHledani,	nadpis: 'Výsledky hledání' } );
